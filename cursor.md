...
1) What did you change in this project?
  ...
  - Updated `.gitignore` to exclude generated report outputs.
...
5) Checklist to apply same improvements to another repository
  ...
  - 7) Add CI workflow: checkout, setup-python, pip install, register ipykernel, run pytest, execute notebook with nbconvert.
  - 8) Unify plot sizes and add code comments explaining statistical reasoning.
  - 9) Update README to follow consistent portfolio style across repos.
  - 10) Rename output artifacts to `report.*` and update `.gitignore`.
  - 11) Fix any notebook validation issues (missing metadata on display_data outputs) before committing.
